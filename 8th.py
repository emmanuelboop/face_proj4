...
generator =  Generator() #torch.load("models/8th_generator") #
...
for epoch in range(100000000):
    ...
    in_imgs = torch.tensor(np.random.uniform(0,1,(64, 784))).float()
    ...
    a = 64#np.random.randint(64, n_imgs)
    indcs = np.random.randint(0, n_imgs, a).tolist()

    in_imgs2 = switch_tiles(train_imgs[indcs].reshape(a, 1, 28, 28), (1, 1))
    in_imgs2 = torch.tensor(in_imgs2).float().reshape(a,784)[:64]
    
    out_imgs2 = compare_images(in_imgs2, train_imgs)

    optim_g.zero_grad()
    rtimgs2 = generator.appr1(in_imgs2)
    r_loss2 = loss_func(rtimgs2, out_imgs2)
    r_loss2.backward()
    optim_g.step()

    if epoch % 1000 == 0:
        rtimgs = torch.cat([rtimgs1, rtimgs2]) #rtimgs2
        out_imgs = torch.cat([out_imgs, out_imgs2])
        noisy_imgs = torch.cat([in_imgs, in_imgs2])
        losses = [r_loss1.item(), r_loss2.item()] # r_loss2.item()
        ...
        #quit()
        torch.save(generator, "models/8th_generator")
